...
health_sec_system <- health %>% select(`Cyber security role` : `Additional disclosure of information`) 
```

```{r warning=FALSE, message=FALSE}
(health %>% group_by(`Critical Industry`) %>%
   summarise(count = n()))

(health %>% filter(!is.na(`Organisation size`)) %>%
    group_by(`Organisation size`) %>%
    summarise(count = n()))

(health %>% group_by(`Level of digital intensity`) %>%
   summarise(count = n()))

for (feature in names(health_sec_system)) {
  health_sec_system %>% filter(!is.na(!!as.symbol(feature))) %>%
...
```
<!--realizzare ananlisi -->
 <!--vedere legame presente tra il numero di record estratti e la dimensione dell'organizzazione / livello di digital intensity / data sensitivity (confronta demo diamonds.Rmd)-->
```{r warning=FALSE, message=FALSE}
...
queensland_factorized <- queensland %>%
  select(`Organisation size`, `Number of users affected`) %>%
  filter(!is.na(`Number of users affected`)) %>%
  filter(!is.na(`Organisation size`)) %>%
